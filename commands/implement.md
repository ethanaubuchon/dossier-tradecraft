...
5. **Execute (inline).** Develop to the approved plan in the main session. Tests are written alongside and verified — **never test-first; no TDD by default.** Keep the plan current as a living document if the approach shifts.
   - **Verification is repo-defined.** Source the verify/test command — and what "build", "test", "lint" actually *mean* — from the repo's agent-instruction file; don't assume a host-native command or a default verb meaning. In **containerized** repos the in-container typecheck/test is the source of truth: run it from inside the worktree dir (bind-mounts are relative — running from the repo root silently tests `main`), make sure the worktree has the gitignored secrets it needs (see `repo-setup`'s **Containerized verification seam** override), and **distrust host LSP/build diagnostics** (e.g. "cannot find module") — they're noise when deps live in the container.
